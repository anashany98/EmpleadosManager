...
import { Paperclip, Plane, X } from 'lucide-react';
...
import { ABSENCE_TYPES, type VacationRequest } from './types';
...
export function VacationSelfServiceView() {
    ...
    const fetchRequests = useCallback(async () => {
        ...
        try {
            ...
            const used = myVacations
                .filter((item: VacationRequest) => item.status === 'APPROVED')
                .reduce((sum: number, item: VacationRequest) => sum + (item.days || 0), 0);
            const pending = myVacations
                .filter((item: VacationRequest) => item.status === 'PENDING')
                .reduce((sum: number, item: VacationRequest) => sum + (item.days || 0), 0);
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden">
                            ...
                            <form onSubmit={handleCreate} className="p-6 space-y-4">
                                ...
                                <div className="pt-2">
                                    <button type="submit" className="w-full py-4 bg-indigo-600 text-white font-bold rounded-xl shadow-xl shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:-translate-y-0.5 transition-all">
                                        Enviar solicitud
                                    </button>
                                </div>
                            </form>
