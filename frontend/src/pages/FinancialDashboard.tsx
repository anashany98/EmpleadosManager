...
import {
  ...
  Activity, CheckCircle, AlertTriangle, Users,
  Plus, Calendar
} from "lucide-react";
...
export default function FinancialDashboard() {
  ...
  const stats = [
    ...
    {
      title: "Horas Extra",
      value: getNumericValue(metrics?.overtime?.hoursThisMonth),
      subtitle: "Este mes",
    ...
    },
    {
      ...
      value: getNumericValue(metrics?.payroll?.totalCost),
      subtitle: "Total",
    ...
    {
      title: "Presupuesto",
      value: getNumericValue(metrics?.payroll?.budget),
      subtitle: "Asignado",
  ...
  const quickActions = [
    { icon: <FileText size={20} className="text-blue-600" />, label: "Importar", href: "/import", bgClass: "bg-blue-50 dark:bg-blue-900/20" },
    { icon: <Activity size={20} className="text-emerald-600" />, label: "Informes", href: "/reports", bgClass: "bg-emerald-50 dark:bg-emerald-900/20" },
    { icon: <Clock size={20} className="text-amber-600" />, label: "Fichajes", href: "/timesheet", bgClass: "bg-amber-50 dark:bg-amber-900/20" },
    { icon: <Users size={20} className="text-violet-600" />, label: "Empleados", href: "/employees", bgClass: "bg-violet-50 dark:bg-violet-900/20" }
  ];
            ...
            <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold uppercase tracking-wide">Resumen Financiero</h3>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-white/10 rounded-xl backdrop-blur-sm">
                    ...
                    <div>
                      <p className="text-sm font-bold">Costes Laborales</p>
                      <p className="text-xs text-white/70">{getNumericValue(metrics?.payroll?.totalCost)} €</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 bg-white/10 rounded-xl backdrop-blur-sm">
                    ...
                    <div>
                      <p className="text-sm font-bold">Horas Extra</p>
                      <p className="text-xs text-white/70">{getNumericValue(metrics?.overtime?.hoursThisMonth)} h</p>
                    </div>
                  </div>
                </div>
              </div>
