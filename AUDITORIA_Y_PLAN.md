...
| --- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| N1  | ✅ CERRADO (`45d4ae4`)                | `backend/src/services/AuditService.ts:36-43`                                                                                                                 | Si la inserción del log de auditoría falla tras 3 reintentos, se llama `log.fatal({ auditData: data })`. Eso puede llevar PII (DNI, salario, acciones sobre empleados) a Sentry/Elasticsearch sin scrubbing. Hay que filtrar campos sensibles antes del log. |
...
| N5  | ✅ CERRADO (`45d4ae4`) — junto con N1 | Mismo que N1, además `auditData` puede contener `metadata` que el llamador añadió sin sanitizar. Defensa: schema Zod o lista blanca de campos serializables. |
...
| N9  | ✅ CERRADO (`ff63010`)                | `backend/src/scripts/debug-*` (×11)                                                                                                                          | 11 scripts de debug en `backend/src/scripts/` que se incluyen en `dist/`. Si no se usan en CI, añadirlos a `.dockerignore` o `tsconfig.exclude`.                                                                                                             |
