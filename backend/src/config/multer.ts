    ...
    const storage = multer.diskStorage({
        destination: (req, file, cb) => {
            ...
            if (req.body && req.body.employeeId) {
                ...
                const safeId = req.body.employeeId.replace(/[^a-zA-Z0-9-]/g, '');
                if (safeId !== req.body.employeeId) {
                    return cb(new Error('Invalid Employee ID Format'), '');
                }
                finalPath = path.join(dest, `EXP_${safeId}`);
            }
